...
| **Active worktrees** | None. The first two tickets of PRD #237 landed 2026-09-23 in one merge train: #257 (#247, `scripts/trace.sh` and ADR-0008 — the kit now traces its own decisions under `.trace/`, gitignored) and #259 (#246, the Claude Code hook facts and the redacted fixtures #251 needs). Both carried an independent cross-vendor review dispatched through the kit's own dispatcher; the reviewer's sandbox had no network, so each review was relayed to the PR by hand. Open from that wave: #248–#255, frontier #248 and #253. Still open from before: a cross-vendor Gemini review end to end; `ai-review.example.yml` is still inert. |
