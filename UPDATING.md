...
`constitution/shared-invariants.md` alone), updating to **0.18.0** (by which point
...
$ # step 5 — apply
  ...
  NOTE  UPDATING.md changed in v0.18.0 — RE-READ IT before continuing
...
$ sh scripts/check.sh
OK  docs gate: all checks passed (shared-layer 0.18.0, engine: docs harness)
$ sed -n 's/^shared-layer:[[:space:]]*//p' VERSION
0.18.0
Part 1 complete — shared layer at v0.18.0. The update is not done: go to step 8.
```
...
`MERGE` is the 0.4.0 → 0.18.0 case for this file, and `ADD` is the 0.3.0 → 0.18.0
...
1: its `VERSION` says 0.18.0 and `scripts/agents.lib.sh` is on disk — and the gate
...
> two refs — a real `v0.3.0 → v0.18.0` clone prints more lines than the fixture
...
ADD     scripts/agents.config.sh is new at v0.18.0 — nothing of ours to preserve
...
**`ADD     scripts/agents.config.sh is new at v0.18.0`.** The tier→model map did
