...
9. **Differential loop.** Run the same input through old-version vs new-version (or two configurations) and diff outputs.
10. **Human-in-the-loop script.** Last resort. If a human must click, drive _them_ with `.agents/skills/diagnose/hitl-loop.template.sh` so the loop is still structured. Captured output feeds back to you.
...
*Adapted from `engineering/diagnose` in [mattpocock/skills](https://github.com/mattpocock/skills) — MIT, see `.agents/skills/LICENSE-mattpocock-skills.md`. Tool names in the loop list were generalised, and the architectural hand-off at the end names this kit's `/improve-codebase-architecture` and the decision record it produces.*
