...
> - **Code craft**: `constitution/shared-code-craft.md`. The portable rules the GREEN and refactor steps are held to — smallest sufficient diff, named constants, flat control flow, named states over boolean parameters, explicit blocks, no drive-by edits. The review's standards axis reads with that file open; write with it open too.
