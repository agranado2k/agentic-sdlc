...
#      (shared-layer 0.1.0) is diffed against a newer kit ($KITV) and updated
...
# The release this kit currently IS, read from the manifest rather than typed.
# Thirty-six literals of one version used to live in this file, so every
...
case "$LAST_OUT" in
*"shared-layer $KITV"*) pass "gate reports shared-layer $KITV" ;;
...
#   kit v$KITV — the kit as it stands here: §9/§10 tightened, and UPDATING.md
...
if cmp -s "$OLDKIT/constitution/shared-invariants.md" "$KIT/constitution/shared-invariants.md"; then
	fail "the fake 0.1.0 rulebook is identical to $KITV — the scenario has no delta"
else
	pass "fake 0.1.0 rulebook differs from $KITV"
fi

# A .git-free copy of the kit as it stands: the v$KITV release tree.
NEWKIT="$SCRATCH/kit-$KITV"
t_kit_tree "$KIT" "$NEWKIT"

# The two ends, as real git refs in one repo.
HIST="$SCRATCH/kit-history"
t_kit_history "$HIST" "$OLDKIT" v0.1.0 "$NEWKIT" "v$KITV"
cd "$HIST" || exit 2
git init -q -b main
...
cp -R "$NEWKIT/." "$HIST/"
git add -A >/dev/null
git commit -q -m "release $KITV"
git tag "v$KITV"
if git rev-parse -q --verify v0.1.0 >/dev/null && git rev-parse -q --verify "v$KITV" >/dev/null; then
	pass "kit history built with tags v0.1.0 and "v$KITV""
else
...
assert_has "VERSION" "shared-layer: $KITV"
...
assert_has "$SCRATCH/part1.transcript" "UPDATING.md changed in v$KITV — RE-READ IT"
...
if git -C "$HIST" diff --quiet v0.1.0 "v$KITV" -- scripts/check.sh; then
...
# on a 0.3.0 -> $KITV update lands the capability-tier RESOLVER (shared layer)
...
for f in .claude/skills/implement/SKILL.md .claude/skills/to-tickets/SKILL.md \
	...
	if cmp -s "$OLD3/$f" "$KIT/$f"; then
		fail "the fake 0.3.0 $f is identical to $KITV — no delta to adopt"
	else
		pass "fake 0.3.0 $f differs from $KITV"
	fi
done
...
t_kit_history "$HIST3" "$OLD3" v0.3.0 "$NEWKIT" "v$KITV"
cd "$HIST3" || exit 2
git init -q -b main
git config user.name "Kit Release"
git config user.email "kit@example.invalid"
...
git commit -q -m "release $KITV"
git tag "v$KITV"
pass "kit history built with tags v0.3.0 and "v$KITV""
...
manifest1 "v$KITV" | sort >"$WORK1/to.list"
# shellcheck disable=SC2046  # manifest paths, one per line, none with spaces
kit1 archive "v$KITV" -- $(cat "$WORK1/to.list") | tar -x
kit1 show "v$KITV:VERSION" >VERSION
git add -A >/dev/null
git commit -q -m "chore: update shared layer 0.3.0 -> $KITV"
...
step6_check kit1 "v$KITV" "$WORK1/to.list" >"$SCRATCH/step6.clean"
...
step6_check kit1 "v$KITV" "$WORK1/to.list" >"$SCRATCH/step6.mode"
...
if kit1 show "v$KITV:scripts/agents.lib.sh" | cmp -s - scripts/agents.lib.sh; then
...
recipe2() {
	WORK=$WORK1
	kit() { kit1 "$@"; }
	# NOT re-derived from VERSION: step 5 already moved it to $KITV. Part 2 runs
...
kit1 archive "v$KITV" .agents/skills/dogfood | tar -x
ln -s ../../.agents/skills/dogfood .claude/skills/dogfood
kit1 show "v$KITV:constitution/local-product.md.template" \
...
assert_block '^mkdir -p \.agents/skills' "$SCRATCH/migrate.sh" \
	"UPDATING.md's $KITV migration block is extractable"
