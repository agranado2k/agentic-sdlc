...
#      (shared-layer 0.1.0) is diffed against a newer kit (0.18.0) and updated
...
case "$LAST_OUT" in
*"shared-layer 0.18.0"*) pass "gate reports shared-layer 0.18.0" ;;
*) fail "gate did not report shared-layer 0.18.0: $LAST_OUT" ;;
esac
...
#   kit v0.18.0 — the kit as it stands here: §9/§10 tightened, and UPDATING.md
...
if cmp -s "$OLDKIT/constitution/shared-invariants.md" "$KIT/constitution/shared-invariants.md"; then
	fail "the fake 0.1.0 rulebook is identical to 0.18.0 — the scenario has no delta"
else
	pass "fake 0.1.0 rulebook differs from 0.18.0"
fi

# A .git-free copy of the kit as it stands: the v0.18.0 release tree.
NEWKIT="$SCRATCH/kit-0.18.0"
t_kit_tree "$KIT" "$NEWKIT"

# The two ends, as real git refs in one repo.
HIST="$SCRATCH/kit-history"
t_kit_history "$HIST" "$OLDKIT" v0.1.0 "$NEWKIT" v0.18.0
cd "$HIST" || exit 2
git init -q -b main
...
cp -R "$NEWKIT/." "$HIST/"
git add -A >/dev/null
git commit -q -m "release 0.18.0"
git tag v0.18.0
if git rev-parse -q --verify v0.1.0 >/dev/null && git rev-parse -q --verify v0.18.0 >/dev/null; then
	pass "kit history built with tags v0.1.0 and v0.18.0"
else
...
assert_has "VERSION" "shared-layer: 0.18.0"
...
assert_has "$SCRATCH/part1.transcript" "UPDATING.md changed in v0.18.0 — RE-READ IT"
...
if git -C "$HIST" diff --quiet v0.1.0 v0.18.0 -- scripts/check.sh; then
...
# on a 0.3.0 -> 0.18.0 update lands the capability-tier RESOLVER (shared layer)
...
# carries today's layout (symlinks over .agents), so un-move it — a fixture
# holding the new layout would make every old-layout recipe path vacuous.
# The doc layer un-moves with it: today's manuals name .agents/skills paths,
# and a reference into a directory this consumer never had is a path-missing
# violation its "green before the update" premise cannot carry.
rm -rf "$OLD3/.claude/skills"
cp -R "$KIT/.agents/skills" "$OLD3/.claude/skills"
rm -rf "$OLD3/.agents"
...
for f in .claude/skills/implement/SKILL.md .claude/skills/to-tickets/SKILL.md \
	...
	if cmp -s "$OLD3/$f" "$KIT/$f"; then
		fail "the fake 0.3.0 $f is identical to 0.18.0 — no delta to adopt"
	else
		pass "fake 0.3.0 $f differs from 0.18.0"
	fi
done

# The un-move sweep runs LAST: the manufactured files above were derived
# from today's tree and re-import .agents references a 0.3.0 tree never had.
find "$OLD3" \( -name '*.md' -o -name '*.md.template' \) -type f \
	-exec grep -l '\.agents/skills' {} + 2>/dev/null |
	while IFS= read -r f; do
		sed 's|\.agents/skills|.claude/skills|g' "$f" >"$f.unmove" && mv "$f.unmove" "$f"
	done

# Both ends as real refs in one repo, exactly as B0 does it.
HIST3="$SCRATCH/kit-history-3"
t_kit_history "$HIST3" "$OLD3" v0.3.0 "$NEWKIT" v0.18.0
cd "$HIST3" || exit 2
git init -q -b main
git config user.name "Kit Release"
git config user.email "kit@example.invalid"
...
git commit -q -m "release 0.18.0"
git tag v0.18.0
pass "kit history built with tags v0.3.0 and v0.18.0"
...
manifest1 v0.18.0 | sort >"$WORK1/to.list"
# shellcheck disable=SC2046  # manifest paths, one per line, none with spaces
kit1 archive v0.18.0 -- $(cat "$WORK1/to.list") | tar -x
kit1 show "v0.18.0:VERSION" >VERSION
git add -A >/dev/null
git commit -q -m "chore: update shared layer 0.3.0 -> 0.18.0"
...
step6_check kit1 v0.18.0 "$WORK1/to.list" >"$SCRATCH/step6.clean"
...
step6_check kit1 v0.18.0 "$WORK1/to.list" >"$SCRATCH/step6.mode"
...
if kit1 show "v0.18.0:scripts/agents.lib.sh" | cmp -s - scripts/agents.lib.sh; then
...
recipe2() {
	WORK=$WORK1
	kit() { kit1 "$@"; }
	# NOT re-derived from VERSION: step 5 already moved it to 0.18.0. Part 2 runs
	...
	kit_take "$TO_REF" ".agents/skills/to-tickets/SKILL.md" "$WORK/theirs"
	...
	echo "\$ ln -s ../../.agents/skills/improve-codebase-architecture .claude/skills/improve-codebase-architecture"
...
take_before=$(wc -c <"$SCRATCH/take-fixture/mine")
{
	...
	cat "$SCRATCH/take.sh"
	echo 'kit_take v0.18.0 no/such/path/at/this/ref "$WORK/mine" && echo "TOOK" || echo "declined to write"'
...
# window loses it forever, gate green. Since 0.12.0 VERSION carries a skills:
...
# with a pre-0.12.0 VERSION (files: only, no skills:) and the block must say
...
kit1 archive v0.18.0 .agents/skills/dogfood | tar -x
ln -s ../../.agents/skills/dogfood .claude/skills/dogfood
kit1 show "v0.18.0:constitution/local-product.md.template" \
...
assert_block '^mkdir -p \.agents/skills' "$SCRATCH/migrate.sh" \
	"UPDATING.md's 0.18.0 migration block is extractable"
